...
function PlayerBall(parentNode, stickyNodes, ballOpts, sounds) {
	...
	this.init = function () {
		...
		canvas_el.style.cssText = 'position: absolute; z-index: 999999999;';
	...
	function drawAttached(att) {
		...
		att.el.style.zIndex = (oz > 0) ? 1000000000 : 999999998;
...
function whenAllLoaded(gameDiv, popup, stickyNodes) {
	...
	jQuery('<button>Start!</button>').click(function () {
		...
		ballOpts = {
			...
			MAX_ATTACHED_VISIBLE: parseInt(Math.round(jQuery('#maxAtt').val()).clamp(1, 9999)),
